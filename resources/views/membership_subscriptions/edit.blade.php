...
@section('title', 'Edit Membership Subscription')
@section('subtitle', 'Update membership subscription details')
...
<div class="mb-4">
    <h1 class="text-xl font-bold text-gray-100">Edit Membership Subscription</h1>
</div>

<div class="bg-slate-800 border border-slate-700 rounded-lg p-4">
    <form method="POST" action="{{ route('management.membership-subscriptions.update', $membershipSubscription) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="customer_id" id="customer_id"
                    ...
                    @foreach($customers as $customer)
                        <option value="{{ $customer->customer_id }}" {{ old('customer_id', $membershipSubscription->customer_id) == $customer->customer_id ? 'selected' : '' }}>
                ...
                <select name="plan_id" id="plan_id"
                    ...
                    @foreach($plans as $plan)
                        <option value="{{ $plan->plan_id }}" 
                                ...
                                {{ old('plan_id', $membershipSubscription->plan_id) == $plan->plan_id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="date" name="start_date" id="start_date" 
                       value="{{ old('start_date', $membershipSubscription->start_date->format('Y-m-d')) }}"
                       class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-sm text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                       required>
                @error('start_date')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="date" name="end_date" id="end_date" 
                       value="{{ old('end_date', $membershipSubscription->end_date->format('Y-m-d')) }}"
                       class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-sm text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                       required>
                @error('end_date')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
            ...
            <div class="md:col-span-2">
                ...
                <select name="status" id="status"
                        class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-sm text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <option value="active" {{ old('status', $membershipSubscription->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="pending" {{ old('status', $membershipSubscription->status) == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="cancelled" {{ old('status', $membershipSubscription->status) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                    <option value="expired" {{ old('status', $membershipSubscription->status) == 'expired' ? 'selected' : '' }}>Expired</option>
                </select>
                @error('status')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
        ...
        <div class="flex items-center justify-end space-x-3 mt-6 pt-4 border-t border-slate-600">
            <a href="{{ route('management.membership-subscriptions.show', $membershipSubscription) }}" 
